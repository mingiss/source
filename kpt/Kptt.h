...
// ----------------------------------
// #define NO_LTCOD // apibrëþti projekto parametruose - seniems projektams, nenaudojantiems Vyèio kodavimo
...
#ifdef NO_LTCOD
#define KPT_UniPrivateStart 0xE000 // Vyèio kodø nenaudojam - kaip seniau
#else
#define KPT_UniPrivateStart 0xE100 // pradþia jau uþimta - Vytis accented lit. chars 0xE000..0xE01A
#endif
// #define KPT_UniPrivateEnd 0xF900  // MS Word uses codes >= 0xF000
#define KPT_UniPrivateEnd 0xEFFF

// pasirinkimas tarp naujø (Vytis) ir senø (KPT_UniPrivateStart + <Kpt code>) lietuviðkø akcentuotø raidþiø kodø
#ifdef NO_LTCOD
...
typedef enum
{
   ...
   C_Bpoint,         // 429 /* bullet, didelis taskas vidury */ // naudojamas homonimø identifikavimui (DOUBLE_ENTRIES_KPCHAR), tikram bullet'ui naudoti C_hBpoint
...
//------------------------- accented cyrillic
   ...
   C_hBpoint,        // 787 /* bullet, didelis taskas vidury */ // tiems atvejams, kai C_Bpoint naudojamas homonimø identifikavimui (DOUBLE_ENTRIES_KPCHAR) 
...
// ----------------------- old style cyrillic
   ...
   C_checkmark,      // 792 // varnelë paþymëjimui
...
} KpChar_enum; // former Chars16; // actually 32 bit signed integers

typedef int KpChar;


/* =================================================== */
extern const KpChar iNullStr[];

/* =================================================== */
extern KpChar *Ctptr7[Ctnum*2];   /* 7-biciu Kodu lenteliu adresu poru sarasas,
                                    Null,Null - lenteles nera */
extern KpChar *Ctptr8[Ctnum*2];   /* 8-biciu Kodu lenteliu adresu poru sarasas */


/* =================================================== */
// Kpt lenteliu pavadinimai; former iaKptCtabExpl
// look lpszaXmlCtblNames[] as well
extern const unsigned char *lpszaKptCtabExpl[KPT_Ctnum+1];

/* =================================================== */
extern KpChar KptUpLowTbl[];     /* upper-/lowercase conversion table */
// must be entered all letter characters, independently, have they lower/upper case or not
extern KpChar KptLatUpLowTbl[];  /* upper-/lowercase conversion table for latin letters */
extern KpChar KptToLatTbl[C_CharSize0]; /* table for conversion to simple latin letters */
extern KpChar KptNoAccTbl[C_CharSize0]; // table for lithuanian accents skipping

/*------------------------------------*/
extern UniChar iaUniChars[C_CharSize0];  /* Unicode character codes */

// ---------------- max length of Kpt Unicode strings
#define KPT_UNI_STR_LEN 3
// converting of Kpt style characters to Kpt strings convertable to Unicode
// use only for visualisation purposes, do not convert master documents
extern KpChar iaszUniStrs[C_CharSize0][KPT_UNI_STR_LEN+1];

/*------------------------------------*/
/* ar atskyrejas?                     */

extern int w_delim cargs((int simb));
extern int c_delim cargs((int simb));


/*------------------------------------*/
extern HRESULT KptCharDecode(unsigned char *pcOutChar, KpChar uiInChar, int iKptTbl);
                     // does not output error messages if the character is not
                     //    found in given code table, returns KP_E_UNKN_CHR or KP_E_FONT_UNDEF only
extern HRESULT KptCharEncode(KpChar *puiOutChar, unsigned char cInChar, int iKptTbl);


extern HRESULT KptStrDecode(unsigned char *lpszOutStr, const KpChar *uiaMsg, /* int iMsgLen, */ int iKptTbl);
                     // does not output error messages if the character is not
                     //    found in given code table, returns KP_E_UNKN_CHR or KP_E_FONT_UNDEF only
                     // adds Nul to the end of encoded lpszOutStr
extern HRESULT KptStrEncode(KpChar *uiaMsg, const unsigned char *lpszInStr, /* int *piMsgLen, */ int iKptTbl);
                     // adds C_Nul to the end of encoded uiaMsg

extern HRESULT KptStrToUnicode(UniChar *saMsg, const unsigned char *lpszInStr, /* int *piMsgLen, */ int iKptTbl);
extern HRESULT KptUnicodeToStr(unsigned char *lpszOutStr, const UniChar *saMsg, /* int iMsgLen, */ int iKptTbl);

extern HRESULT KptKpCharStrToUnicode(UniChar *saMsg, const KpChar *iaInStr, bool bDotLessToDot);
// bDotLessToDot == True - convert C_i_Acute_Dotless to C_i_Acute, etc.
extern HRESULT KptUnicodeToKpCharStr(KpChar *iaOutStr, const UniChar *saMsg);

// extern HRESULT KptUTF8ToUnicode(UniChar *saMsg, const unsigned char *lpszInStr); // netrealizuota dar; kol kas naudoti DecodeKpStrUnicode()

extern HRESULT EncodeWideStr(unsigned char *lpszStrBuf, long lBufLen, int iKptTbl); // aðtuonbièius kodus uþkoduoja L"" formatu – keièia á unicode \x0117  
